...
std::pair<double, numcpp::objects::Vector> fg(const numcpp::objects::Vector& v) {

    double x = v(0), y = v(1);
    numcpp::objects::Vector grad(2);
    grad(0) = -2.0 * (1.0 - x) - 400.0 * x * (y - x * x);
    grad(1) =  200.0 * (y - x * x);
    return std::make_pair(std::pow(1.0 - x, 2) + 100.0 * std::pow(y - x * x, 2), grad);
}
...
void testBFGSRosenbrock() {
    ...
    auto result3 = numcpp::optim::bfgs(fg, x0);
    assert(std::abs(result2.x(0)-1.0)<=1e-5);
    assert(std::abs(result2.x(1)-1.0)<=1e-4);
    assert(std::abs(result2.f)<=1e-10);
